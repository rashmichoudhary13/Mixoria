...
const Menu = () => {
  ...
  return (
    <section className='relative max-h-dvh my-20 mb-50'>
       <img src={sliderLeftLeaf} alt="" className='absolute w-30 md:w-fit -bottom-90 md:-bottom-20 object-contain'/>
       <img src={sliderRightLeaf} alt="" className='absolute right-0  -top-40 md:-top-50 w-30 md:w-fit'/>

       <nav className='grid grid-cols-2 md:grid-cols-4 mx-6 md:max-w-6xl md:mx-auto gap-10 md:gap-20 relative z-10 mt-30'>
        {
       ...
       <div className='radial-gradient flex justify-between relative mx-6 md:mx-20'>
         ...
         <div className='absolute flex justify-between items-center container max-w-7xl mt-20'>
            <button className='hover:text-yellow-100 cursor-pointer transition-colors max-w-20 text-2xl'
                ...
                <span className='hidden md:block'>{prevCocktail.name}</span>
                <img src={prevArrow} alt="" />
            </button>

            <button className='hover:text-yellow-100 cursor-pointer transition-colors max-w-20 text-2xl'
                    onClick={() => goToSlide(currentIndex + 1)}>
                <span className='hidden md:block'>{nextCocktail.name}</span>
         ...
         {/* Recipe */}
         <div className='flex flex-col md:flex-row absolute justify-between w-full gap-9 md:gap-10 bottom-0 md:items-end translate-y-60 md:translate-y-0'>
            <div className='space-y-2'>
                <h2 className='text-sm'>Recipe for: </h2>
                <p className='text-2xl md:text-5xl md:w-1/2 text-yellow-100 title'>{currentCocktail.name}</p>
            </div>

            <div className='max-w-sm space-y-4 details'>
